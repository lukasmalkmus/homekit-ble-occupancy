...
GOFLAGS += -ldflags='-s -w -extldflags "-fno-PIC -Wl"
...
.PHONY: install
install: $(GOPATH)/bin/homekit-ble-occupancy ## Install the binary into the $GOPATH/bin directory
...
$(GOPATH)/bin/homekit-ble-occupancy: dep.stamp $(call go-pkg-sourcefiles, ./...)
	@echo ">> installing homekit-ble-occupancy binary"
	@$(GO_BIN_IN_PATH) install $(GOFLAGS) ./cmd/homekit-ble-occupancy
